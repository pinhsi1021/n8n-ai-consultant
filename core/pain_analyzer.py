...
DATA_SOURCE_MAP = {
    ...
    "IoT/感測器":["iot", "感測器", "sensor", "設備", "機台", "溫度", "濕度", "壓力"],
    "表單系統":  ["表單", "google form", "問卷", "typeform"],
    "檔案系統":  ["檔案", "文件", "pdf", "圖片", "照片", "影像", "掃描"],
...
ACTION_MAP = {
    "預測分析":   ["預測", "predict", "forecast", "趨勢", "未來", "預估"],
    ...
    "統計彙總":   ["統計", "彙總", "加總", "平均", "計算", "彙整", "aggregate"],
    "排程優化":   ["排程", "排班", "調度", "時程", "排序", "優化", "最佳化"],
    "風險評估":   ["風險", "評估", "評分", "scoring", "信用", "風控"],
...
OUTPUT_MAP = {
    ...
    "資料儲存":       ["儲存", "記錄", "歸檔", "存檔", "備份", "log"],
    "自動回覆":       ["自動回覆", "自動回信", "chatbot", "機器人", "客服"],
...
COMPLEXITY_MAP = {
    ...
    "多系統整合":    ["多系統", "多平台", "整合", "串接", "異質系統"],
    "合規/安全":     ["合規", "法規", "gdpr", "個資", "安全", "加密", "隱私"],
    "機器學習":      ["機器學習", "ml", "模型", "訓練", "深度學習", "ai模型"],
...
def _build_summary(keywords, sources, actions, outputs, complexity, industry, focus):
    ...
    parts = []
    if industry:
        parts.append(f"針對「{industry}」產業")
    if focus:
        parts.append(f"的「{focus}」問題")

    if keywords:
        parts.append(f"，核心關鍵字為「{'、'.join(keywords[:5])}」")

    if actions:
        parts.append(f"。系統偵測到您需要的處理能力：{'、'.join(actions)}")
